...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    const char* shaderSource = R"(
        ...
        PS_INPUT VS_Main(VS_INPUT input)
        {
            float1 degToRad = 3.14f / 180;
            

            PS_INPUT output;
                    
            float3 finalPos = input.pos;
            float2 rotate2 = float2(finalPos.x, finalPos.y);
        
            float2 rotate3 = Rotate2D(rotate2, g_Offset.x*degToRad);
            finalPos = float3(rotate3, 0.5f);
            output.pos = float4(finalPos, 1.0f);

            float4 coutputCol = float4(g_Offset.x,g_Offset.x,g_Offset.x,1);
            coutputCol = coutputCol / 360;

            output.col = coutputCol;
            return output;
        }
    ...
    while (WM_QUIT != msg.message) {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            printf("%f, ", g_CurOffset.x);
            if (GetAsyncKeyState(VK_LEFT)) {
                g_CurOffset.x -= rotateSpeed;
            }
